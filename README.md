...
**Известное ограничение.** Проверено с рабочим ключом на трёх разных целях, без события и с EV5: сейчас `source` всегда приходит `fallback`. Причина не в ключе, а в модели: на `gpt-6-sol` через `/v1/chat/completions` вызовы функций отклоняются с ошибкой 400 «Function tools with reasoning_effort are not supported... set reasoning_effort to 'none'», а `run_agent` перехватывает любое исключение и уходит в запасной путь. Пока это не исправлено на стороне API, ответ гарантированно валиден и считается движком, но цель пользователя на подбор плана не влияет.
...
Всего 195 тестов: 178 выполняются, 17 пропускаются до появления `POST /api/agent` в `api/main.py`. Для `tests/test_api.py` нужны установленные зависимости, поэтому запускать следует из виртуального окружения, созданного `./run.sh`, либо после `source .venv/bin/activate` командой `python -m pytest -q`.
