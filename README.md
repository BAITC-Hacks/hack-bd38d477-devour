...
- **События:** информационный экран. В текущих `AGENTS.md`, `engine/` и `api/main.py` нет событий, `GET /api/events` и параметра `event_id`. Бюджет остаётся 100.
...
Вклад меры — `Score(полный набор) − Score(набор без этой меры)`. Это предельный вклад, поэтому сумма вкладов не обязана совпадать с общей дельтой. Движок считает без округления; интерфейс отображает до двух знаков. В текущем API численные ответы не округляются.
...
- События и изменение бюджета событиями пока отсутствуют. Параметр `event_id` не отправляется.
