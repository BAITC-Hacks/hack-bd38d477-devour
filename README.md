...
| AI-аким | Есть. `POST /api/agent`: модель подбирает набор инструментами `validate_plan` и `simulate_plan`, движок проверяет итог; с ключом отвечает `source: "ai"` (проверено), без ключа — план оптимизатора |
...
Две витрины работают на одном `web/app.js` и одном API. Основная — `/`, вторая — `/v2/` (`web-v2/`), она описана в конце раздела.
...
`web-v2/index.html` сохраняет все id и классы, которые использует `app.js`, поэтому расчёты, события, сравнение и оптимизатор работают там так же, как на `/`. Отличается подача: верхняя навигация; главная собирается слоями — панорама Астаны, огромная надпись «АСТАНА» шрифтом Manrope, вырезанный Байтерек поверх букв, затем HTML-текст, золотая кнопка «Начать управление», Score и бюджет и полоса пяти районов с их проблемами из кейса. После расчёта с положительной дельтой панорама «оживает»: два световых слоя плавно меняют прозрачность, ожидание API показано отдельно. На экране решений — карточки мер с фотографией направления и строкой «Начало эффекта: … квартал», справа тёмная панель «Доступно / Выбрано / Остаток» с полосой расходов, пятью пронумерованными слотами и причинами блокировки из `/api/validate`; событие показывается локальным уведомлением и отдельным сегментом потери бюджета. Результат — «Город стал лучше» или «Город изменился», крупный Score с плашкой дельты, тепловая карта и «Как рассчитано» в раскрывающихся блоках, AI-анализ блоками «Что улучшилось», «Чем пришлось пожертвовать», «Что осталось критичным», «Что попробовать иначе». AI-аким — баннер, цель, лента настоящих шагов агента и тёмный «План действий». Карту районов и полосу расходов рисует `web-v2/map.js`, ленту агента — `web-v2/agent.js`. Фотографии на рабочих экранах занимают не больше четверти площади; вёрстка адаптивна до 375 px, `prefers-reduced-motion` отключает анимации.

## Архитектура и технологии

Python 3.11+ и FastAPI на сервере, статические HTML/CSS/JavaScript без сборки на клиенте. Ниже — полный перечень сторонних компонентов, которые реально используются в коде, с версиями из `requirements.txt` и `pip list`.

| Компонент | Версия | Где используется |
| --- | --- | --- |
| FastAPI | 0.141.1 | `api/main.py`: маршруты, валидация тел запросов, ошибки 422 |
| Starlette | 1.7.0 | Зависимость FastAPI: `StaticFiles`, `FileResponse`, `JSONResponse`, `TestClient` |
| Pydantic | 2.13.5 | Зависимость FastAPI: модели `DecisionsRequest`, `Scenario`, `CompareRequest`, `AgentRequest` |
| Uvicorn | 0.53.0 | ASGI-сервер: `run.sh`, Dockerfile, ручной запуск |
| OpenAI Python SDK | 3.19.0 | `ai/explainer.py`, `ai/agent.py`: Chat Completions с `response_format` и вызовом функций |
| Модель OpenAI | из `OPENAI_MODEL`, по умолчанию в коде `gpt-6-sol` | Объяснение результата, сравнение сценариев и AI-аким |
| python-dotenv | 1.2.3 | `api/main.py`: чтение `.env` |
| httpx | 0.28.1 | Транспорт `fastapi.testclient` в интеграционных тестах |
| pytest | 9.1.1 | Все тесты |
| Google Fonts: Onest | — | `web/styles.css`, `web-v2/styles.css` (интерфейс) |
| Google Fonts: Manrope | — | `web/styles.css` (весь интерфейс `/`), `web-v2/styles.css` (крупные заголовки, Score, «АСТАНА») |

С CDN подключаются только шрифты (`fonts.googleapis.com`, `fonts.gstatic.com`); других внешних скриптов, стилей, библиотек графиков или карт в `web/` и `web-v2/` нет — при недоступности сети интерфейс переходит на системный шрифт и продолжает работать. Движок `engine/` использует только стандартную библиотеку Python.

**Происхождение данных.** Районы, десять показателей, веса, 14 мер, синергии и несовместимости — синтетический датасет из материалов кейса Astana Innovations, перенесённый в `engine/data.json` без изменений. Пять событий EV1–EV5 в `engine/events.json` придуманы командой. Изображения в `web-v2/assets/` сгенерированы командой с помощью AI (ChatGPT) для этого проекта — это не документальные фотографии и не чужая графика. Чисел от модели в расчётах нет: всё считает движок.
...
├── Dockerfile             образ на python:3.12-slim, uvicorn на 0.0.0.0:8000
├── .dockerignore          .env, .venv, .git и служебные файлы не попадают в образ
├── .github/workflows/tests.yml  ручной прогон pytest в GitHub Actions
...
├── api/
│   └── main.py            FastAPI: эндпоинты, округление до двух знаков, отдача web/ и /v2/
├── ai/
│   ├── explainer.py       объяснение результата через OpenAI, проверка чисел, шаблонный разбор
│   ├── advisor.py         советник: перебор замены одной меры с проверкой движком
│   ├── agent.py           AI-аким: цикл инструментов validate_plan / simulate_plan
│   └── test_numeric_grounding.py  28 тестов защиты от выдуманных чисел
├── web/                   витрина /
...
├── web-v2/                витрина /v2/ на фотографиях, тот же app.js
│   ├── index.html         слои главной, слоты решений, уведомления о событии
│   ├── styles.css         токены «Астана. Новый день», Onest и Manrope
│   ├── map.js             карта районов и полоса расходов
│   ├── agent.js           лента шагов и план действий AI-акима
│   ├── DESIGN.md          дизайн-система витрины
│   └── assets/            9 изображений, сгенерированных командой с помощью AI
└── tests/
    ├── test_engine.py     99 тестов движка
    ├── test_api.py        96 тестов HTTP-эндпоинтов
    └── test_ai.py         14 тестов AI-ветки с подменой клиента OpenAI
```
...
## AI-аким: `POST /api/agent`

Отдельный режим, в котором план собирает не человек, а модель: получив свободную цель, она сама перебирает наборы мер и проверяет каждый движком.

Запрос — `{"goal": str | null, "event_id": str | null}`. Оба поля необязательны: пустой объект `{}` принимается и означает цель по умолчанию, неизвестный `event_id` даёт 422 с русским текстом ошибки.

Ответ:

| Поле | Содержимое |
| --- | --- |
| `decisions` | Итоговый набор из пяти мер |
| `simulation` | Полный результат `simulate` для этого набора с учётом события |
| `steps` | Журнал обращений к движку: `action` (`validate` или `simulate`), проверенный набор и краткий итог на русском |
| `explanation` | Текстовое обоснование плана |
| `source` | `ai` — план собрала модель, `fallback` — план собран движком без модели |

**Инструменты.** Модели доступны три функции из `ai/agent.py`, и все они ведут в движок: `list_measures` отдаёт меры, районы и бюджет из `engine/data.json`, `validate_plan` прогоняет набор через `validate`, `simulate_plan` сначала проверяет набор, затем считает `simulate`. Каждый вызов `validate_plan` и `simulate_plan` попадает в `steps`, поэтому по ответу видно, какие наборы модель перебрала и чем они закончились.

**Ограничение шагов.** `TOOL_LIMIT = 8`: после восьми обращений к инструментам цикл прерывается. Если к этому моменту модель не вернула финальный JSON, ответ собирается запасным путём.

**Fallback.** Запасной путь включается, если ключа нет, если обращение к OpenAI завершилось ошибкой, если модель вернула некорректный JSON или если предложенный ею набор не прошёл валидацию. Тогда план берётся из `optimize(top=1, event_id=...)`, проверяется `validate`, считается `simulate`, а текст собирается шаблонным разбором. Числа в обоих режимах считает движок, отличается только источник плана и формулировки.

```bash
curl -s http://localhost:8000/api/agent \
  -H 'Content-Type: application/json' \
  -d '{"goal": "подтянуть Нуру", "event_id": null}'
```

Ответ приходит примерно за секунду и содержит набор из пяти мер, `simulation` со `score` и `total_cost`, два шага журнала и обоснование. С событием запрос выглядит так же, только `"event_id": "EV5"`, и тогда набор подбирается под уменьшенный бюджет.

**Состояние.** Проверено с рабочим ключом: на цель «подтянуть Нуру» без события эндпоинт отвечает `source: "ai"` за 10–20 секунд, журнал содержит около десяти настоящих шагов `validate`/`simulate`, итоговый набор валиден, а числа в объяснении совпадают с результатами инструментов. Модель вызывается с `reasoning_effort="none"`, потому что `gpt-6-sol` через Chat Completions не принимает вызовы функций с рассуждениями. Если лимит из восьми вызовов исчерпан, модель получает отдельный запрос без инструментов и обязана выбрать лучший из уже проверенных планов; объяснение с числами, которых нет в результатах инструментов, отправляется на одну переработку, а при повторной ошибке ответ собирается из лучшего проверенного плана с пометкой `fallback`.
...
Всего 237 тестов: 99 движка, 96 API, 14 AI-ветки с подменой клиента OpenAI и 28 тестов защиты от выдуманных чисел в `ai/`. Ни один не обращается в сеть. Для `tests/test_api.py` нужны установленные зависимости, поэтому запускать следует из виртуального окружения, созданного `./run.sh`, либо после `source .venv/bin/activate` командой `python -m pytest -q`.
...
`tests/test_api.py` — 96 интеграционных тестов через `fastapi.testclient`, включая 17 тестов `POST /api/agent` (контракт ответа, валидность набора, совпадение Score с `simulate`, бюджет с событием, 422 на неизвестное событие). Фикстура убирает `OPENAI_API_KEY` из окружения, поэтому анализ заведомо идёт шаблонным путём и тесты не обращаются в сеть. Проверяются состав `/api/state`, ошибки валидации, контрольный Score в `/api/simulate`, код 422 на невалидном наборе, все шесть полей анализа и `source: "fallback"` в `/api/explain`, ответы `/api/compare` и `/api/optimize`, отдача интерфейса на `/`, а также то, что все числа с плавающей точкой в ответах округлены не более чем до двух знаков.
...
`tests/test_ai.py` — 14 тестов AI-ветки без сети: клиент OpenAI подменяется фальшивым объектом со сценарием ответов. Для объяснения: валидный ответ → `source: "ai"`; невалидный JSON → один повтор с указанием на ошибку схемы → при второй неудаче `fallback`; предложение с выдуманным числом отбрасывается, а если после чистки текста не осталось — `fallback`; ошибка провайдера → `fallback`; без ключа клиент не создаётся. Для AI-акима: цепочка `list_measures → validate_plan → simulate_plan → финальный JSON` даёт `source: "ai"` и настоящие шаги; при лимите в восемь вызовов инструментов модель получает запрос на финальный ответ без инструментов; невалидный итоговый набор заменяется лучшим из проверенных; выдуманные числа в объяснении отправляются на переработку; ошибка провайдера даёт валидный план оптимизатора с учётом события.
